...
var ICVExportJsonKeeper = function(config) {
    ...
    function _exportCurationJson(curation, idToken, options) {
        ...
        if ($modals.length === 1) {
            ...
            $modals.spin();
            $modalContent = $modals.children('.modal-content');
            if ($modalContent.length === 0) {
                $modalContent = $modals.children('.sidebar-content');
            }
        }
        var settings = {
            method: method,
        ...
        $.ajax(settings).done(function(data, textStatus, jqxhr) {
            if (jqxhr) {
                ...
                if (resLocation) {
                    ...
                    if (method === 'POST') {
                        ...
                        if (target === '_self') {
                            var duration = 1500;
                            showAlert($modalContent, 'success', (lang !== 'ja') ? 'Exported as New Curation' : '新規にエクスポートしました', duration);
                            setTimeout(function() {
                    ...
                    showAlert($modalContent, 'success', (lang !== 'ja') ? 'Updated' : '完了', 1000);
                }
                if (options.callback) {
            ...
            showAlert($modalContent, 'danger', error || ((lang !== 'ja') ? 'Error' : 'エラー'), 3000);
